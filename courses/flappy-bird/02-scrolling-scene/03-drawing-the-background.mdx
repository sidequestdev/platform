...
There's nothing _wrong_ with storing your images separately if that's what you'd like to do. We just like the idea of having a single image that contains all of the sprites, and a single metadata file that holds information about the images within. Where are they located? What are the dimensions? etc.
:::

Here's a diagram to give you a better idea:

![Sprite sheet diagram](./img/spritesheet-explainer-dark.png)

:::alert{title="What is a Sprite?" type="info"}
If you aren't familiar with the term _sprite_, think of it as a 2d bitmap image. We'll use the terms image and sprite interchangeably in this course.
:::
...
![Frame diagram](./img/frame-diagram-dark.png)
...
Update `main.ts` to render the background:

```typescript {4,7-18} title="src/main.ts"
